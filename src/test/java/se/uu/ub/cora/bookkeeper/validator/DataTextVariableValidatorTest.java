...
public class DataTextVariableValidatorTest {
	...
	@Test(expectedExceptions = ConfigurationException.class, expectedExceptionsMessageRegExp = "Error while validating data. The regular expression (.|\\n|\\r)+ caused a stack overflow.")
	...
	private DataElementValidator setUpRegexpWithCatastrophicBacktracking() {
		textVariable = TextVariable.withIdAndNameInDataAndTextIdAndDefTextIdAndRegularExpression(
				"id", "nameInData", "textId", "defTextId", "(.|\\n|\\r)+");
		DataElementValidator textDataValidator = new DataTextVariableValidator(textVariable);
	...
	private DataElementValidator setUpRegexpWithSyntaxError() {
		textVariable = TextVariable.withIdAndNameInDataAndTextIdAndDefTextIdAndRegularExpression(
				"id", "nameInData", "textId", "defTextId", "(badRegEx");
		DataElementValidator textDataValidator = new DataTextVariableValidator(textVariable);
		return textDataValidator;
	}
	...
	private String getLongDummyString() {
		return """
				The beauty of autumn lies in its subtle transition between warmth and cold. The vibrant colors of red, orange, and yellow leaves blanket the ground, creating a vivid contrast with the crisp air. As the days shorten, there's a sense of nostalgia in the atmosphere, reminding us of the fleeting nature of time. It's a season that invites reflection, where the outdoors seem to slow down, and we too feel compelled to pause and enjoy the moment.

				In cities, autumn often brings a shift in pace. Streets once bustling with the energy of summer are now quieter, as people retreat indoors to find warmth. Parks, once filled with picnics and games, now offer a more solitary experience, perfect for long walks and moments of solitude. This is a time when many seek comfort in books, hot drinks, and the cozy ambiance of home, allowing for a deeper connection to one's thoughts and surroundings.

				For nature, autumn marks a critical time of preparation. Animals gather food and build shelters in anticipation of the coming winter. Trees shed their leaves, conserving energy for the colder months. The cycle of life becomes more visible as plants and creatures adapt to the changing environment, demonstrating resilience and the natural balance of the ecosystem. It's a reminder of the interconnectedness of all living things.

				As autumn progresses, the anticipation of winter grows. The crisp mornings turn colder, and the first frost may appear. Yet, there's something comforting in the predictability of the seasons, a rhythm that has guided human life for centuries. We know that after the stillness of winter, a new spring will emerge, full of life and possibility. Until then, autumn offers a gentle transition, allowing us to prepare for the next chapter.
				""";
	}
}

...
public class RecordTypeHandlerImp implements RecordTypeHandler {
	...
	public String getNextId() {
		...
		// SPIKE STARTS
		if (sequenceIdSource()) {
			DataRecordLink sequenceLink = recordType
					.getFirstChildOfTypeAndName(DataRecordLink.class, "sequence");
			IdSource idSource = idSourceFactory.factorSequenceIdSource(recordStorage,
					sequenceLink.getLinkedRecordId(), getCreateDefinitionId());
			return idSource.getId();
		}
		throw new DataMissingException(
				"Unable to get next id, no valid idSource exists in recordType");
		// SPIKE ENDS
	}

	private boolean sequenceIdSource() {
		return ("sequence").equals(recordType.getFirstAtomicValueWithNameInData(ID_SOURCE));
	}
